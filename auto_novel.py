...
def train(model, train_loader, labeled_eval_loader, unlabeled_eval_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, ((x, x_bar),  label, idx) in enumerate(tqdm(train_loader)):
            ...
            label=label.data.to(torch.int64)
...
def test(model, test_loader, args):
    ...
    acc, nmi, ari = util.cluster_acc_test(targets.astype(int), preds.astype(int)), nmi_score(targets, preds), ari_score(targets, preds) 
    print('Test acc {:.4f}, nmi {:.4f}, ari {:.4f}'.format(acc, nmi, ari))

if __name__ == "__main__":
    test2='Y:\\space-visualstudio\\workplace_python\\DL\\9\\PythonApplication1'
    import argparse
    ...
    parser.add_argument('--dataset_root', type=str, default=test2+'\\data\\datasets\\CIFAR\\')
    parser.add_argument('--exp_root', type=str, default=test2+'\\data\\experiments\\')
    parser.add_argument('--warmup_model_dir', type=str, default=test2+'\\data\\experiments\\pretrained\\auto_novel\\resnet_rotnet_cifar10.pth')
